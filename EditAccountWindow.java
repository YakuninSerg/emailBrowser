...
public class EditAccountWindow extends JFrame {
    ...
    private String pop_security;
    private String pop_port;
    private String imap_host;
    private String imap_security;
    ...
    EditAccountWindow(String account) {
        super("Редактирование учетной записи");
        ...
        JTextField userTextField = new JTextField(account);
        userTextField.setEnabled(false);

        JLabel emailLabel = new JLabel("Адрес почты");
        JTextField emailTextField = new JTextField(user);
        JLabel atLabel = new JLabel("@");

        emailComboBox.setPreferredSize(new Dimension(100, 20));
        initEmailComboBox();

        emailComboBox.setSelectedItem(mailEnd);
        emailComboBox.setEditable(true);

        JLabel loginLabel = new JLabel("Логин");
        JTextField loginTextField = new JTextField(user);

        JLabel passwordLabel = new JLabel("Пароль");
        JPasswordField passwordField = new JPasswordField(pass);

        JLabel smtpServerLabel = new JLabel("Адрес сервера");
        JTextField smtpServerTextField = new JTextField(smtp_host);

        JLabel smtpSaveConnectionLabel = new JLabel("Защита соединения");
        JTextField smtpSaveConnectionTextField = new JTextField(smtp_security);

        JLabel smtpPortLabel = new JLabel("Порт");
        JTextField smtpPortTextField = new JTextField(smtp_port);

        JLabel pop3ServerLabel = new JLabel("Адрес сервера");
        JTextField pop3ServerTextField = new JTextField(pop_host);

        JLabel pop3SaveConnectionLabel = new JLabel("Защита соединения");
        JTextField pop3SaveConnectionTextField = new JTextField(pop_security);

        JLabel pop3PortLabel = new JLabel("Порт");
        JTextField pop3PortTextField = new JTextField(pop_port);

        JLabel imapServerLabel = new JLabel("Адрес сервера");
        JTextField imapServerTextField = new JTextField(imap_host);

        JLabel imapSaveConnectionLabel = new JLabel("Защита соединения");
        JTextField imapSaveConnectionTextField = new JTextField(imap_security);

        JLabel imapPortLabel = new JLabel("Порт");
        JTextField imapPortTextField = new JTextField(imap_port);
        ...
        emailComboBox.addActionListener(e -> {
            ...
            if (file.exists()) {
                ...
                try (InputStream is = new FileInputStream(filePath.toString())) {
                    ...
                    JOptionPane.showMessageDialog(EditAccountWindow.this,
    ...
    private void initFields() {
        String propertyFile = SendWindow.PROPERTIES_FOLDER + "//" + account + ".properties";
        try (InputStream is = new FileInputStream(propertyFile)) {
            Reader reader = new InputStreamReader(is);
            Properties propertiesFromFile = new Properties();
            propertiesFromFile.load(reader);
            smtp_host = propertiesFromFile.getProperty("smtp_host");
            smtp_security = propertiesFromFile.getProperty("smtp_security");
            smtp_port = propertiesFromFile.getProperty("smtp_port");

            pop_host = propertiesFromFile.getProperty("pop_host");
            pop_security = propertiesFromFile.getProperty("pop_security");
            pop_port = propertiesFromFile.getProperty("pop_port");

            imap_host = propertiesFromFile.getProperty("imap_host");
            imap_security = propertiesFromFile.getProperty("imap_security");
            imap_port = propertiesFromFile.getProperty("imap_port");

            user = propertiesFromFile.getProperty("user");
            pass = propertiesFromFile.getProperty("pass");
            ;
            replyTo = propertiesFromFile.getProperty("reply_to");
            from = propertiesFromFile.getProperty("from");

            mailEnd = from.substring(from.lastIndexOf("@") + 1);
        } catch (FileNotFoundException e) {
            JOptionPane.showMessageDialog(EditAccountWindow.this,
                    "Файл учетной записи " + account + " не найден!",
                    "Ошибка",
                    JOptionPane.ERROR_MESSAGE);
            e.printStackTrace();
        } catch (IOException e) {
            JOptionPane.showMessageDialog(EditAccountWindow.this,
                    "Ошибка при чтении файла " + propertyFile,
                    "Ошибка",
                    JOptionPane.ERROR_MESSAGE);
            e.printStackTrace();
        }
    ...
    public static void main(String[] args) {
        new EditAccountWindow("Альфаброк");
    }
}
